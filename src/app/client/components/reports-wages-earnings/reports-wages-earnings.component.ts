...
import { DataDriven, WagesEarningsPayCodes } from '../../models';
import { ReportsWagesEarningsService, ReportsService } from '../../services';

@Component({
  selector: 'app-reports-wages-earnings',
  templateUrl: './reports-wages-earnings.component.html',
  styleUrls: ['./reports-wages-earnings.component.scss']
})
export class ReportsWagesEarningsComponent implements OnInit {

  @Input() employeeId?: string;
  @Input() pageSource?: string;

  summaryCheckboxGroup = {
    yes: false,
    no: true
  };
  ...
  reportsWagesEarningsReportData: WagesEarningsPayCodes;
  filteredWagesEarningsReportData: WagesEarningsPayCodes[];
  filteredWagesEarningsPayCodesDataList: WagesEarningsPayCodes[];

  reportsWagesEarningsReportSummaryData: WagesEarningsPayCodes;
  filteredWagesEarningsPayCodesSummaryData: WagesEarningsPayCodes[];
  ...
  employeeDisplaySsn: string;
  payCodeDisplayDesc: DataDriven[];
  ...
  constructor(
    ...
    private reportsWagesEarningsService: ReportsWagesEarningsService,
  ...
  viewReport() {
    ...
    if (this.pageSource !== 'employeeReports') {
      ...
      if (this.noAnswered === false) {

      this.reportsWagesEarningsService.getWagesEarnings()
      .subscribe(reportsWagesEarningsReportData => {
          this.filteredWagesEarningsReportData = reportsWagesEarningsReportData;

      this.filteredWagesEarningsReportData = this.filteredWagesEarningsReportData.filter(e => ((this.selectedLocationCode.ID === 'LOC.ALL'
      || e.locationCodeID === this.selectedLocationCode.ID) &&
          ...
          (this.selectedDepartmentCode.ID === 'DEP.ALL' || e.departmentCodeID === this.selectedDepartmentCode.ID) &&
        ...
        this.reportsWagesEarningsService.getWagesEarningsSummary()
        .subscribe(reportsWagesEarningsReportSummaryData => {
            this.filteredWagesEarningsPayCodesSummaryData = reportsWagesEarningsReportSummaryData;

        this.filteredWagesEarningsPayCodesSummaryData = this.filteredWagesEarningsPayCodesSummaryData
        .filter(e => ((this.selectedLocationCode.ID === 'LOC.ALL'
        || e.locationCodeID === this.selectedLocationCode.ID) &&
            (this.selectedDivisionCodes.ID === 'DIV.ALL' || e.divisionCodeID === this.selectedDivisionCodes.ID) &&
            (this.selectedDepartmentCode.ID === 'DEP.ALL' || e.departmentCodeID === this.selectedDepartmentCode.ID) &&
            (this.selectedPayCode.ID === 'PAY.ALL' || e.payCodeID === this.selectedPayCode.ID)) &&
    ...
    if (this.pageSource === 'employeeReports' && this.employeeId.length > 0) {
